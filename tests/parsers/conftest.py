...
PLATFORM_A_HEADER = (
    ...
    "金流手續費,平台手續費,金流處理費,退款日期,退款金額,應收款項(淨額),"
    "結算日期,撥款日期"
)
...
def platform_a_source() -> SettlementSource:
    rows = [
        PLATFORM_A_HEADER,
        # 單純銷售：三個手續費欄相加 = 55.30
        "2026-03-01,SP-24A7X9K2,EC0001,信用卡,5.53%,無線滑鼠,1000,"
        "30.42,19.35,5.53,,0.00,944.70,2026-03-13,2026-03-15",
        # 編號小寫，正規化後應該還原
        "2026-03-02,sp-24b8y1m3,EC0002,信用卡,5.53%,機械鍵盤,2500,"
        "76.04,48.39,13.82,,0.00,2361.75,2026-03-14,2026-03-16",
        # 編號整欄空白 → external_order_id 必須是 None，不是空字串
        "2026-03-03,,EC0003,ATM,5.53%,USB 集線器,680,"
        "20.68,13.16,3.76,,0.00,642.40,2026-03-15,2026-03-17",
        # 退款寫在同一列 → 這一列要拆成「銷售 + 退款」兩筆
        "2026-03-04,SP-24D1A3P5,EC0004,信用卡,5.53%,螢幕支架,2400,"
        "73.00,46.45,13.27,2026-03-22,600.00,1667.28,2026-03-16,2026-03-18",
    ]
    return source("platform_a_2026-03.csv", "\n".join(rows) + "\n")


@pytest.fixture
def platform_a_inconsistent_source() -> SettlementSource:
    """淨額欄與各費用欄算出來的結果對不上的一列。

    模擬「對帳檔多了一個我們沒讀到的扣款欄位」——parser 必須讓這一列失敗，
    而不是產生一筆少扣費用、看起來正常的記錄。
    """
    rows = [
        PLATFORM_A_HEADER,
        # 三費相加 55.30，1000 - 55.30 = 944.70，但檔案宣告 900.00
        "2026-03-01,SP-24A7X9K2,EC0001,信用卡,5.53%,無線滑鼠,1000,"
        "30.42,19.35,5.53,,0.00,900.00,2026-03-13,2026-03-15",
    ]
    return source("platform_a_broken.csv", "\n".join(rows) + "\n")
